...
except ImportError:  # only `check` and `unmerge` need these
    wordsegment = None

    def zipf_frequency(*_a, **_k):
        raise SystemExit(
            "`check` and `unmerge` need the word libraries:\n"
            "    pip install wordsegment wordfreq\n"
            "`reflow` does not, and works without them.")
...
def main():
    ...
    a = ap.parse_args()
    if a.cmd in ("check", "unmerge"):
        if wordsegment is None:
            zipf_frequency()          # raises the install hint
        wordsegment.load()
    if a.cmd == "check":
